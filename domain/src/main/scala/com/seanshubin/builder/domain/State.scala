...
object State {
  ...
  case class Processing(statusOfProjects: StatusOfProjects) extends State {
    override def handlePartially(): PartialFunction[Event, (State, Seq[Effect])] = {
      ...
      case UpdatesWereApplied(projectName) => update(projectName, ProjectState.Upgraded, Effect.AddAfterUpdate(projectName))
      case UpdatesAdded(projectName) => update(projectName, ProjectState.AddedUpgrade, Effect.CommitAfterUpdate(projectName))
      case UpdatesCommitted(projectName) => update(projectName, ProjectState.CommittedUpgrade, Effect.PushAfterUpdate(projectName))
