...
trait RunnerWiring {
  ...
  lazy val homeDirectory:Path = Paths.get(userHome).resolve(Paths.get("github", "sean"))
  ...
  lazy val systemApi: SystemApi = new SystemApiImpl(systemExecutor, homeDirectory, environment, notifications)
